...
{
  return (float)(randVector[index]) ;
}
#pragma OPENCL EXTENSION cl_khr_fp64:enable
double randD(__global float* randVector , int index)
{
  return (double)(randVector[index]) ;
}
                                 ...
                                 __global  int* tetrapositions, float minExpectedQ, int mxIter, int subI, __global float* randVector)
{
	   ...
	   //calculo la calidad minima del set
		...
		for (iter = 0 ; iter<mxIter ; iter++)
		{
			...
			{
				randX =  rand(randVector, (vertexID+0)%5000);
				randY =  rand(randVector, (vertexID+1)%5000);
				randZ =  rand(randVector, (vertexID+2)%5000);
				proposed = (float4)( (randX-0.5)*radius,(randY-0.5)*radius,(randZ-0.5)*radius,0.0) + initialPos ;
