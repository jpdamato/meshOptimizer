...
int smoothTime , byEdgetime, byFaceTime, byNodeTime;
...
{
	int evaluations = 0;
	byNodeTime = byEdgetime = byFaceTime= smoothTime = 0;
	...
	for (int j=0; j<numIter;j++)
		{
			...
			if (flag % SMOOTH_MESH == 0) 
			{
				...
				startProcess("SMOOTH_MESH");
				stopTimers();
				if (!isGPUEnabled())
				  evaluations +=ParallelSmoothMesh(m,vrelaxQuality,minExpQ);
				else
				  evaluations +=ParallelSmoothMeshInGPU(m,vrelaxQuality, minExpQ);
				startTimers();
				smoothTime += endProcess("SMOOTH_MESH");
			}
...
void benchmark0(std::string meshFileName,int numIter, double minExpQ ,int flag,float dt)
{
	cout <<".............................................." <<"\n"; 
	cout <<".........BENCHMARK 0 ........................." <<"\n"; 
	cout <<" Loading Mesh..." <<"\n"; 
	...
	for (int i=0 ; i<1 ; i++)
	{
	  ...
	  qt->refresh();   qt->print();
	 // myfile << i<<"\t" <<isGPUEnabled() <<" "<<omp_get_num_threads() <<" "<<m->elements->Count()<<"\t"<<minD <<"\t"<<origAve <<"\t"<<minExpQ <<"\t" <<qt->nonPositive<<"\t" <<flag << "\t"<< qt->fMinQuality <<"\t" <<qt->fDieMin <<"\t" <<qt->DieAveMin<<"\t"<<smoothTime<<"\t"<<byEdgetime<<"\n"<<byFaceTime<<"\t"<<byNodeTime<<"\n";
	}
	...
	myfile << meshFileName<<"\t" <<isGPUEnabled()<<"\t"<<omp_get_num_threads() <<"\t"<<m->elements->Count()<<"\t"<<minD <<"\t"<<origAve <<"\t"<<minExpQ <<"\t" <<qt->nonPositive<<"\t" <<flag <<"\t" << t<< "\t"<< qt->fMinQuality <<"\t" <<qt->fDieMin <<"\t" <<qt->DieAveMin<<"\t"<<smoothTime<<"\t"<<byEdgetime<<"\n"<<byFaceTime<<"\t"<<byNodeTime<<"\n";
...
void benchmark2(std::string meshFileName, float dt, int maxIter, int numMejoras)
{
	...
	for (int i=0 ;i<maxIter ; i++)
	{
		...
		optimizeMesh(m,numMejoras,220, APPLY_BY_FACE * APPLY_BY_EDGE  * APPLY_BY_NODE * SMOOTH_MESH);
...
/*
	...
	{	
		...
		ml->save(meshFileName + "_Moved_"  +  intToStr(i) + ".vwm", m);
	...
	{	
		...
		myfile<<i<<"\t"<<et<<"\t"<<qt->fFaceMin<<"\t"<<qt->fDieMax<<"\t"<<qt->fDieMin<<"\t"<<qt->nonPositive<<"\n";
		ml->save(meshFileName + "_Delaunnay_"  +  intToStr(i) + ".vwm", meshes[i]);
	...
	*/
	...
	{	
		...
		startProcess("Mesh Optimization");
		stopTimers();
        optimizeMesh(m,4,18 ,APPLY_BY_EDGE * APPLY_BY_FACE);
     	startTimers();			
	    double et= endProcess("Mesh Optimization");		
		...
		if (qt->nonPositive >0 )
		{
		 ...
		 // delete m;
		  ...
		  qt = new TetQuality(m);
		  qt->refresh();   qt->print();
		  appliedDelaunnay = true;
		}

		if (appliedDelaunnay) 
		  ml->save(meshFileName + "_Opt_Delaunnay_"  +  intToStr(i) + "_D.vwm", m);
		else
		  ml->save(meshFileName + "_Opt_Delaunnay_"  +  intToStr(i) + ".vwm", m);	 
	}
    myfile.close();
...
int main (int argc, char* argv[])
{
	...
	if (numbenchmark == 0)
	{
		myfile.open ("benchmark_Journal0.txt");
		myfile<<"benchmark_Journal0"<<"\n";		
		// None GPU 1 thread
		setNumThreads(1);setGPUEnabled(false);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);
		// None GPU 2 thread
		setNumThreads(2);setGPUEnabled(false);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);	
		// None GPU 4 thread
		setNumThreads(4);setGPUEnabled(false);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);
		// None GPU 6 thread
		setNumThreads(6);setGPUEnabled(false);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);

		// None GPU 8 thread
		setNumThreads(8);setGPUEnabled(false);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);


		// GPU 1 thread
		setNumThreads(1);setGPUEnabled(true);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);
		// GPU 2 thread
		setNumThreads(2);setGPUEnabled(true);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);	
		// GPU 4 thread
		setNumThreads(4);setGPUEnabled(true);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);
		// None GPU 6 thread
		setNumThreads(6);setGPUEnabled(true);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);

		// GPU 8 thread
		setNumThreads(8);setGPUEnabled(true);
		benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.005);
		...
		{
			//------------------------------------------------------------
			//---- JPDC Journal : Benchamark 1 . Different Meshes
			//------------------------------------------------------------
			myfile.open ("benchmark_1.txt");
			myfile<<"benchmark1"<<"\n";		

			setNumThreads(1);
			setGPUEnabled(false);
			benchmark0("dragon",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH  ,0.0);
			benchmark0("sculpt10kv",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH  ,0.0);
			benchmark0("staypuft",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH  ,0.0);
			benchmark0("fiat_cil",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.0 );	
			benchmark0("benchmark0_cavityMesh_0_00_600k",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.0 );
			benchmark0("tire",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH  ,0.0);
		...
		else
		{
			...
			//------------------------------------------------------------
			//---- JPDC Journal : Benchamark 1 . F1
			//------------------------------------------------------------
			setNumThreads(1);	setGPUEnabled(false);
			benchmark0("f1_tetgen_mesh",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH ,0.0 );
	...
	/*
	benchmark7("d://test2//cavityMesh_0_00",100,0.01);
	return 0;
	*/
	/*
	...
	///------------------------------	
	*/
//	benchmark4("benchmark0_cavityMesh_0_00_600k",0.005,10);
//	return 0;
	// Sample 0 : Try to optimize quality
 /*
	myfile.open ("benchmark_0_GPU_Timers.txt");

	setNumThreads(8);
	setGPUEnabled(true);
	benchmark0("benchmark0_cavityMesh_0_00_600kit3",9,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH );
	*/
/*
	setNumThreads(8);
	setGPUEnabled(true);
	benchmark0("benchmark0_cavityMesh_0_00_600kit3",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH );
...
*/
	...
	setNumThreads(1);
	setGPUEnabled(true);
	//benchmark0("tire",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH );
	/*benchmark0("fiat_cil",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH );
	benchmark0("staypuft",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH );
	benchmark0("sculpt10kv",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH );
	benchmark0("dragon",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH );
	benchmark0("benchmark0_cavityMesh_0_00_600k",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH );
	*/
	//benchmark0("f1_tetgen_mesh",3,220 , APPLY_BY_EDGE * APPLY_BY_FACE * APPLY_BY_NODE * SMOOTH_MESH );
	
	benchmark2("benchmark0_cavityMesh_0_00_600k",0.001,100,3);
