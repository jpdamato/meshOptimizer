...
int numOptimizedClusters ;

float* gbrandomVector ;



void resetNumOptClusters()
{
	numOptimizedClusters = 0;
}

int getNumOptClusters()
{
	return numOptimizedClusters;
}
...
float rand( float* randVector , int index)
{
...
{
  return (float)(gbrandomVector[index%5000]) ;
}

void randomize()
{
	gbrandomVector = new float[5000];
	for (int i = 0 ;i<5000 ;i++)
		 gbrandomVector[i] = (float)( 1.0f*rand() / RAND_MAX);

}

float* getRandValues()
{
	return gbrandomVector;
}
  ...
  double minq,avgq ,minq2, avgq2 ,q,minVol,radius;
	   ...
	   //calculo la calidad minima del set
		...
		for (iter = 0 ; iter<_gbmxIter ; iter++)
		{
			...
			{
				randX = (double)(rand(i*iter*nproposals*3));// (double)(rand()*1.0/RAND_MAX);
				randY =  (double)(rand(i*iter*nproposals*3+1));// (double)(rand()*1.0/RAND_MAX);
				randZ =  (double)(rand(i*iter*nproposals*3+2));// (double)(rand()*1.0/RAND_MAX);
		...
		*/
		...
		if (mode ==0) 
			...
			if (mode ==1) 
				...
				if (mode ==4)					
					clBestVertexPos(vRes,_gbminExpectedQ,_gbmxIter,_gbsubI,getDefaultKernel());
